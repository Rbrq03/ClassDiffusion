#!/usr/bin/env python
# coding=utf-8
# Copyright 2024 Class Diffusion authors from BJTU and the HuggingFace Inc. team. All rights reserved.
#
...
def save_model_card(
    ...
    model_description = f"""
# Class Diffusion - {repo_id}

These are Class Diffusion adaption weights for {base_model}. The weights were trained on {prompt} using [Class Diffusion](https://www.cs.cmu.edu/~custom-diffusion). You can find some example images in the following. \n
{img_str}
...
class ClassDiffusionDataset(Dataset):
    """
...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="Class Diffusion training script.")
    parser.add_argument(
        "--pretrained_model_name_or_path",
        type=str,
        default=None,
    ...
    parser.add_argument("--use_spl", action="store_true")
    parser.add_argument("--spl_weight", type=float, default=1.0)
    parser.add_argument("--cls_token", type=str)
    parser.add_argument("--tracker_name", type=str, default="custom-diffusion")
...
def main(args):
    ...
    # Add forward_with_custom_embeddings to TextModel
    text_encoder.text_model.forward_with_custom_embeddings = types.MethodType(
        forward_with_custom_embeddings, text_encoder.text_model
    )
    ...
    if args.modifier_token is not None:
        ...
        if args.cls_token is not None:
            ...
            args.cls_token = args.cls_token.split("+")

            if len(args.modifier_token) != len(args.cls_token):
                raise ValueError(
                    "num of Modifier token shoule be the same as the class token"
                )

            modifier_cls_text_inputs = []
            cls_text_inputs = []

            for modifier_token, cls_token in zip(args.modifier_token, args.cls_token):
                print(modifier_token, cls_token)
                modifier_cls_text_input = tokenizer(
                    "a photo of a " + modifier_token + " " + cls_token,
                    padding="longest",
                    return_tensors="pt",
                )

                cls_text_input = tokenizer(
                    "a photo of a " + cls_token,
                    padding="longest",
                    return_tensors="pt",
                )

                modifier_cls_text_inputs.append(modifier_cls_text_input)
                cls_text_inputs.append(cls_text_input)
            ...
            spl_loss = SPL(
                text_encoder=text_encoder, use_attention_mask=use_attention_mask
            )
    ...
    train_dataset = ClassDiffusionDataset(
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet), accelerator.accumulate(text_encoder):
                ...
                if args.with_prior_preservation:
                    ...
                    if accelerator.num_processes > 1:
                        text_embeddings = (
                            text_encoder.module.get_input_embeddings().weight
                        )
                    else:
                        text_embeddings = text_encoder.get_input_embeddings().weight
                    ...
                    for i in range(len(modifier_token_id)):
                        spl_l = (
                            ...
                            + spl_loss(
                                text_embeddings,
                                modifier_token_id[i],
                                modifier_cls_text_inputs[i],
                                cls_text_inputs[i],
                            ).mean()
                        )

                    spl_l = spl_l / len(modifier_token_id)

                    spl_l = args.spl_weight * spl_l
            ...
            if args.use_spl:
                logs["spl"] = spl_l.clone().detach().item() / args.spl_weight
                logs["recon_loss"] = logs["loss"] - logs["spl"] * args.spl_weight
